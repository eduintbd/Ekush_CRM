...
import { CheckCircle, ChevronRight, ChevronLeft, Upload, Loader2, User, FileText, CreditCard, Eye, Sparkles } from "lucide-react";
import Link from "next/link";
import { RegistrationFormPreview } from "@/components/auth/registration-form-preview";

// Optional prefill from a logged-in Tier-1 prospect. When the server
// wrapper detects a PROSPECT session it passes name/phone/email here
// so Step 1 starts populated. Phase 7 — KYC conversion bridge.
...
export type RegisterClientProps = {
  initialProfile?: RegisterPrefill | null;
  // Set when the visitor is a logged-in Tier-1 prospect; surfaces the
  // small "we pre-filled your details" banner above Step 1 and is
  // forwarded to /api/auth/register so the new Investor row can be
  // linked back via Investor.linkedProspectId.
...
export function RegisterClient({ initialProfile, prospectId }: RegisterClientProps = {}) {
  const router = useRouter();
  const [step, setStep] = useState(0);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  // Profile fields. When a Tier-1 prospect lands here logged in, the
  // server wrapper hands us their name/phone/email so Step 1 starts
  // populated. Password is intentionally NOT seeded — we never see
  // the prospect's plaintext password and the user must re-enter one
  // for the investor account.
  ...
  const prefilled = Boolean(prospectId && initialProfile && (initialProfile.name || initialProfile.email || initialProfile.phone));
        ...
        <div className="bg-white rounded-[10px] shadow-card p-8">
          ...
          {step === 0 && (
            <div className="space-y-4">
              ...
              {prefilled && (
                <div className="bg-ekush-orange/10 border border-ekush-orange/30 rounded-[10px] p-3 text-[13px] text-text-dark flex items-start gap-2">
                  <Sparkles className="w-4 h-4 text-ekush-orange shrink-0 mt-0.5" />
                  <span>
                    We pre-filled your details from your prospect profile. Set a password and continue &mdash; you only need to do the document and financial steps from here.
                  </span>
                </div>
              )}
