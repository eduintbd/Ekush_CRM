...
import { CheckCircle, XCircle, Loader2, FileText, Receipt } from "lucide-react";
    ...
    <div className="p-4 space-y-4">
      ...
      <div>
        ...
        {loading ? (
          ...
          <p className="text-text-body text-sm text-center py-6">
          ...
          <div className="space-y-3">
            {pending.map((a) => (
              <div
                key={a.id}
                className="border border-input-border rounded-[10px] p-4 space-y-3"
              >
                  ...
                  <div>
                    ...
                    {a.details && (
                      <div className="mt-2 text-xs text-text-body space-y-0.5">
                        <p>
                          Investor: {a.details.investor?.name} (
                          {a.details.investor?.investorCode})
                        </p>
                        <p>
                          Fund: {a.details.fund?.code} | Direction:{" "}
                          {a.details.direction} | Amount:{" "}
                        ...
                        <p>
                          Units: {Number(a.details.units).toFixed(4)} @ NAV{" "}
                          {Number(a.details.nav).toFixed(4)}
                        </p>
                        {a.details.paymentMethod === "DDI" &&
                        typeof a.details.paymentRef === "string" &&
                        a.details.paymentRef.startsWith("ddi:") ? (
                          <a
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setRejectNotes({ ...rejectNotes, [a.id]: e.target.value })
                    }
      ...
      <div>
        ...
        {processed.length === 0 ? (
          <p className="text-text-body text-sm text-center py-4">
            No processed approvals yet.
          </p>
        ) : (
          <div className="space-y-2">
            {processed.slice(0, 20).map((a) => (
              <div
                key={a.id}
                className="flex items-center justify-between p-3 bg-page-bg rounded-[10px]"
              >
                <div>
                  <div className="flex items-center gap-2">
